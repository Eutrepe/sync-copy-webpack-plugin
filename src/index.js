...
class SyncCopyPlugin {
  constructor(patterns = [], options = {}) {
    if (!Array.isArray(patterns)) {
      throw new Error('[sync-copy-webpack-plugin] patterns must be an array');
    }
  ...
  apply(compiler) {
    ...
    const logger = log({
      name: 'sync-copy-webpack-plugin',
      level: this.options.logLevel || 'warn',
    });

    const plugin = { name: 'SyncCopyPlugin' };
    ...
    compiler.hooks.emit.tapAsync(plugin, (compilation, callback) => {
      ...
      if(this.options.sync) {
        patterns.reduce((chain, pattern) => {
          return chain.then(async () => {
            const pat = await preProcessPattern(globalRef, pattern);
            const files = await processPattern(globalRef, pat);
            ...
            return await Promise.all(
              files
                .filter(Boolean)
                .map((file) => postProcessPattern(globalRef, pat, file))
            );
          });
